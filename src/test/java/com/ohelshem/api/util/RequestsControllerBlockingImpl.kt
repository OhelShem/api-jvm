...
object RequestsControllerBlockingImpl : RequestsController {
    override fun get(url: String, callback: (Response, Result<String, Exception>) -> Unit) {
        url.httpGet().sync().responseString { request, response, result ->
            callback(response, result)
        }
    }

    override fun post(url: String, body: String?, callback: (Response, Result<String, Exception>) -> Unit) {
        url.httpPost().sync().apply { if (body != null) body(body) }.responseString { request, response, result ->
            callback(response, result)
        }
    }
}